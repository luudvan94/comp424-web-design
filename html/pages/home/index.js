    ...
    getJSON("https://apiv2.apifootball.com/?action=get_events&from=2019-11-06&to=2019-11-13&match_id=" + matchId + "&APIkey=b47f6c0ed9a93f917a90078ee60eb94d1d2b5f8048d40d3af159731e53bffdc1")
        .then(response => {
            ...
            goals.forEach(function (goal) {
                ...
                event.push({ "time": time, "event": "goal", "isHomeTeam": goal.home_scorer != "" ? true : false, "content": goal.time + "' - goallllll! by " + scorer });
            });

            cards.forEach(function (card) {
                var time = parseInt(card.time);
                var scorer = card.home_fault != "" ? card.home_fault : card.away_fault;
                event.push({ "time": time, "event": "card", "isHomeTeam": card.home_fault != "" ? true : false, "content": card.time + "' - " + card.card + " for " + scorer });
            });

            substitutions.away.forEach(function (substitution) {
                var time = parseInt(substitution.time);
                event.push({ "time": time, "event": "substitution", "isHomeTeam": false, "content": substitution.time + "' - " + substitution.substitution });
            });

            substitutions.home.forEach(function (substitution) {
                var time = parseInt(substitution.time);
                event.push({ "time": time, "event": "substitution", "isHomeTeam": true, "content": substitution.time + "' - " + substitution.substitution });
            });
